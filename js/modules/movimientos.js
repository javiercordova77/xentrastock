...
window.movimientosModule = {
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <div id="ubicacion-dropdown" class="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto hidden">
    ...
    populateSelects() {
        ...
        // Ubicaciones (solo para filtros, el dropdown personalizado se llena dinámicamente)
    ...
    async updateUbicacionesWithStock(varianteId) {
        ...
        try {
            ...
            // Poblar dropdown con ubicaciones - diseño simplificado sin círculos
            let dropdownHTML = `
                <div class="p-3 bg-blue-50 border-b">
                    <p class="text-xs font-semibold text-blue-700 uppercase tracking-wide flex items-center">
    ...
    selectUbicacion(ubicacionId, ubicacionNombre, stock) {
        const selectedText = document.getElementById('ubicacion-selected-text');
        const ubicacionHidden = document.getElementById('select-ubicacion');
        const dropdown = document.getElementById('ubicacion-dropdown');
        
        if (selectedText && ubicacionHidden) {
    ...
    setupUbicacionDropdownEvents() {
        ...
        if (ubicacionSelector && !ubicacionSelector.hasAttribute('data-events-bound')) {
            ...
            ubicacionSelector.setAttribute('data-events-bound', 'true');
        }
    },
    ...
    resetForm() {
        ...
        // Resetear selector de ubicaciones personalizado
        const selectedText = document.getElementById('ubicacion-selected-text');
        const ubicacionResetInput = document.getElementById('select-ubicacion');
        const dropdown = document.getElementById('ubicacion-dropdown');
        ...
        if (ubicacionResetInput) ubicacionResetInput.value = '';
        if (dropdown) {
            dropdown.innerHTML = `
                <div class="p-3 text-sm text-blue-700 border-b bg-blue-50 flex items-center">
                    <i class="fas fa-info-circle mr-2"></i>
    ...
    // Método load requerido por app.js
    async load() {
        // Resetear estado del módulo
        this.eventsInitialized = false;
        this.isSubmitting = false;
