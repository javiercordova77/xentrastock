 ...
 * Versión con datos reales de ColchonesW
 */

// Variable global para almacenar datos reales transformados
let realInventarioData = [];
            ...
            <div id="salida-modal" class="modal-overlay hidden">
                <div class="modal-content max-w-2xl">
                    ...
                    <form id="salida-form" class="space-y-6">
                        <!-- Same form structure as ingreso -->
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
...
// Load inventario data from ColchonesW API
async function loadInventarioData() {
    try {
        ...
        // Use the transformed data with the existing system
        filteredInventario = realInventarioData;
        renderInventarioTable(realInventarioData);
        updateInventarioStats(realInventarioData);
...
// Load select options using real data
async function loadSelectOptions() {
    try {
        ...
        // For products, we'll populate from the real inventory data once it's loaded
...
// Update variantes select (simplified version for real data)
function updateVariantesSelect() {
    // This function can be simplified since we're working with real data structure
    // For now, we'll keep it simple
    console.log('Variantes select updated');
}

// Update stock info (simplified version for real data)
...
// Render inventario table (maintains original interface)
...
// Get stock status (maintains original logic)
...
// Filter inventario (maintains original interface but uses real data)
function filterInventario() {
    ...
    let filtered = realInventarioData;
...
// Update inventario stats (maintains original interface)
function updateInventarioStats(inventario) {
    ...
    const productos = new Set(inventario.map(item => item.varianteSku)).size;
